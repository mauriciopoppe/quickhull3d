...
Face3Store.prototype.create = function (points, i, j, k) {
  ...
  debug('creating a face with indices: [%j, %j, %j]', i, j, k);
  face.id = this.faceId++;
...
Face3Store.prototype.computeHorizon = function (initialFace, point) {
  ...
  function dfs(face, edgeStart) {
    var i, j;
    var nextFace;
    var commonEdge;
    var neighbors = face.neighbors;
    ...
    for (i = edgeStart, j = 0; j < 3; i = (i + 1) % 3, j += 1) {
      nextFace = neighbors[i];
      if (!nextFace.destroyed) {
        commonEdge = face.commonEdge(nextFace);
        if (vec3.dot(point, nextFace.normal) > nextFace.signedDistanceToOrigin) {
          // the face is able to see the point
          dfs(nextFace, nextFace.indices.indexOf(commonEdge[0]));
        } else {
          ...
          horizonEdges.push(commonEdge);
          horizonFaces.push(nextFace);
        }
      }
    }
  }
